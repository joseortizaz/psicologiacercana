...
export default async function OrgAdminPatientDetailPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: appointments }, { data: therapists }, { data: clinic }] = await Promise.all([
    supabase
      .from("appointments")
      .select(
    ...
    supabase.from("clinics").select("name").eq("id", patient.clinic_id).single<{ name: string }>(),
  ]);

  return (
    <div className="flex flex-col gap-8">
      <div>
        <Link href="/org-admin/patients" className="text-sm text-ink/50 hover:text-ink">
          ← Pacientes
        </Link>
      </div>
